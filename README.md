shell```docker run -it -d --name nginx-external -v /nginx/html:/usr/share/nginx/html:ro -v /nginx/config:/etc/nginx/. -v /nginx/logs:/var/log/nginx/. -p 5555:80 nginx:latest```
